...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    */
    ...
    {
        vec3 v = uv.xyx + iTime + vec3(float(i), float(i+1), float(i+2));
        colors[i] = map(-1.0, 1.0, sin(v), 0.0, 1.0);
    }

    float grayValue = snoise(vec3(noiseScale * uv, iTime));
    grayValue = roundTo(grayValue, 1.0 / float(numColors));

    vec3 col = colors[int(grayValue * float(numColors))];
